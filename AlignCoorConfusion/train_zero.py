...
# 与之前不同是，本次将会把aligned_chains更换成未align的形式
# Author: Jun Li
...
for epoch in range(num_epochs):
    ...
    for index in seed_random_ls:
        ...
        index = int(index)
        global_step += 1
        pred_coor = torch.from_numpy(np.array(train_file["protein"+str(index)]["pred_coor"], dtype=np.float32)).to(device)
        ...
        import time
        beg = time.time()
        pred_coor_c_attn = pred_coor
        pred_coor_r_attn = pred_coor
