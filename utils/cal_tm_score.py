...
if __name__ == '__main__':
    ...
    for i in range(label.shape[0]):
        tmscore = TMscore(pred[i], label[i])
        tm = tm_align(pred[i], label[i], seq, seq)
        print(tmscore)
        print(tm.tm_norm_chain2)
        print("____________")


    # # pred = ((pred.squeeze())[3,:,:]).cpu().numpy()
    # # label = ((label.squeeze())[3,:,:]).cpu().numpy()
    # # print(TMscore(pred, label))

    # pred = pred.squeeze().cpu().numpy()
    # label = label.squeeze().cpu().numpy()
    # tm_list = TMscore_traverse(pred, label)
    # tm_max = tm_list.max()
    # tm_mean = tm_list.mean()
    # print(tm_max, tm_mean)
