...
class CoorNet(nn.Module):
    def __init__(self, x2d_dim=105, clip_num=25, resnet_dim=128, num_blocks=8):
        ...
        self.cmb_2_x2d_II = nn.Sequential(self.b1, self.b2, self.b3)
        self.cmb_2_x2d_III = nn.Sequential(self.b1, self.b2, self.b3)
        self.cmb_2_x2d_IV = nn.Sequential(self.b1, self.b2, self.b3)

        self.x2d_2_coor_I = nn.Sequential(self.b4, self.b5, self.b6)
        self.x2d_2_coor_II = nn.Sequential(self.b4, self.b5, self.b6)
        self.x2d_2_coor_III = nn.Sequential(self.b4, self.b5, self.b6)
        self.x2d_2_coor_IV = nn.Sequential(self.b4, self.b5, self.b6)
    ...
    def forward(self, embed, atten):
        ...
        true_coor_ls.append(coor_)

        coor_clip = coor_2_clip(coor)
        cmb = torch.concat((coor_clip, x2d), dim=1)
        x2d_tmp = self.cmb_2_x2d_II(cmb)
        x2d = (x2d + x2d_tmp) / 2
        x2d = self.norm_x2d(x2d)
        coor_clip_tmp = self.x2d_2_coor_II(x2d)
        coor_clip_tmp = torch.permute(coor_clip_tmp, (0,2,3,1))
        coor = (coor + coor_clip_tmp) / 2
        coor_ = coor.reshape(batch_size, L*L, 3)
        true_coor_ls.append(coor_)
        ...
        coor_clip = coor_2_clip(coor)
        cmb = torch.concat((coor_clip, x2d), dim=1)
        x2d_tmp = self.cmb_2_x2d_III(cmb)
        x2d = (x2d + x2d_tmp) / 2
        x2d = self.norm_x2d(x2d)
        coor_clip_tmp = self.x2d_2_coor_III(x2d)
        coor_clip_tmp = torch.permute(coor_clip_tmp, (0,2,3,1))
        coor = (coor + coor_clip_tmp) / 2
        coor_ = coor.reshape(batch_size, L*L, 3)
        true_coor_ls.append(coor_)
        ...
        coor_clip = coor_2_clip(coor)
        cmb = torch.concat((coor_clip, x2d), dim=1)
        x2d_tmp = self.cmb_2_x2d_IV(cmb)
        x2d = (x2d + x2d_tmp) / 2
        x2d = self.norm_x2d(x2d)
        coor_clip_tmp = self.x2d_2_coor_IV(x2d)
        coor_clip_tmp = torch.permute(coor_clip_tmp, (0,2,3,1))
        coor = (coor + coor_clip_tmp) / 2
        coor_ = coor.reshape(batch_size, L*L, 3)
        true_coor_ls.append(coor_)
        ...
        ### true_coor_ls分别对应了四个block的预测结果，x2d对应了最后一个block对x2d的结果
        return true_coor_ls, x2d
...
if __name__ == "__main__":
    ...
    # torch.Size([4, 16441, 3]) torch.Size([4, 105, 129, 129]) 4
    # pred与label都保持这种形状：[4, 16441, 3]